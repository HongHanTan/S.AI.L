...
- **2026-09-20** — Two model issues found and fixed along the way. `gemini-2.0-flash` is
  retired (the API names `gemini-3.6-flash` as successor), and `gemini-3.6-flash` exhausted
  its free-tier allowance at 20 requests, so the pipeline now runs on
  `gemini-3.5-flash-lite`, which has separate quota and classifies just as well.
- **2026-09-20** — Rate limiting was the whole story behind the 0.4136 dip: a 429 is an
  early request, not a failed one. The client now throttles, recognises quota errors, and
  waits the exact interval the server names.
